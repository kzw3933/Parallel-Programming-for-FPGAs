...
#ifndef _fft_Stage_R_1_H_
#define _fft_Stage_R_1_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "fft_Stage_R_1_memcore.h"

SC_MODULE(fft_Stage_R_1) {
...
protected:
    ...
    // buffer instances
    fft_Stage_R_1_memcore* buffer[BufferCount];
...
public:
    ~fft_Stage_R_1();
    SC_CTOR(fft_Stage_R_1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "fft_Stage_R_1_memcore_U_";
            name += szTmp;

            buffer[i] = new fft_Stage_R_1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "fft_Stage_R_1.trace");
            // ports
